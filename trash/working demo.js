...
function handleParagraphElement(element, requests, pageWidth, pageHeight, lists, slideId) {
  ...
  if(textRuns.length > 1){
    for(var x = 0; x < textRuns.length; x ++ ){
      ...
      if (textRunsSub.content.includes('\u000b')) {
        ...
        resultArray.forEach(function(item,count,index){
          ...
          const lines = item.split(' ').length / 6;
          ...
          // for (const run of textRunsSub) {
            ...
            const style = {
              ...
              link: link || {},
              fontSize: {
                // magnitude: fontSize?.magnitude || 20,
                magnitude: 20,
                unit: 'PT',
              },
              foregroundColor: {
        ...
        const lines = textRunsSub.content.split(' ').length / 6;
        const lineHeight = 20 * 1.2;
        const shapeHeight = lineHeight * lines;
    ...
    if (textRunsOne.content.includes('\u000b') && !isListItem) {
      ...
      const lines = Math.ceil(textRuns.reduce((acc, run) => acc + run.content.split(' ').length, 0) / 6);
      const lineHeight = 20 * 1.2;
      const shapeHeight = lineHeight * lines;
